...
#' @export
plot_gdp_by_category <- function(cat_var, gov_types = NULL, regimes = NULL) {
  if (missing(cat_var)){
  ...
  dat |>
    ...
    ggplot(aes(y = .data[[cat_var]], x = GDP, fill = .data[[cat_var]])) +
    ...
    labs(
      x     = "GDP (log scale)",
      y     = "",
      title = paste("Country GDP in USD (in Billions) by", var_name),
      subtitle = paste("Comparing the GDP of Countries by", var_name)
    ) +
    theme_bw() +
